...
import Testinvoice from "../models/TestInvoiceModal.js";
...
export const addBrand = async (req, res) => {
    try {
        const { name, mobile, email, accountno, bankname, branchname } = req.body;

        const optimized = req.files && req.files.length > 0
            ? await optimizeImages(req.files)
            : [];

        const newBrand = new Testinvoice({
            name, mobile, email, accountno, bankname, branchname,
            image: optimized[0] || ""
        });
...
export const updateBrand = async (req, res) => {
  try {
    ...
    const existing = await Testinvoice.findById(id);
    if (!existing)
    ...
    if (req.files && req.files.length > 0) {
      ...
      image = optimized[0]; // single file
    }

    // Update all fields you need
    const updated = await Testinvoice.findByIdAndUpdate(
      id,
      {
        name: req.body.name ?? existing.name,
        mobile: req.body.mobile ?? existing.mobile,
        email: req.body.email ?? existing.email,
        accountno: req.body.accountno ?? existing.accountno,
        bankname: req.body.bankname ?? existing.bankname,
        branchname: req.body.branchname ?? existing.branchname,
...
export const deleteBrand = async (req, res) => {
  try {
    const { id } = req.params;
    const product = await Testinvoice.findByIdAndDelete(id);
    if (!product) return res.status(404).json({ success: false, message: "Testinvoice not found" });
    res.json({ success: true, message: "Testinvoice deleted" });
  } catch (error) {
...
export const listBrand = async (req, res) => {
  try {
    ...
    const brands = await Testinvoice.find(filter)
      .skip((page - 1) * limit)
      .limit(limit)
      .sort({ createdAt: -1 });

    const total = await Testinvoice.countDocuments(filter);
