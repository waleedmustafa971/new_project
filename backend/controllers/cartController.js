...
export const listFoodCarts = async (req, res) => {
  //{{url}}/api/cart/list?userId=69a2aa0041a6300225b0e7ab
  try {
    console.log(req.query)
    const { page = 1, limit = 10, search = "", categoryId, vendorId, userId } = req.query;
    ...
    if (vendorId) filter.vendorId = vendorId;
    if(userId) filter.userId = userId;
    filter.status = "not yet submit";
    const skip = (page - 1) * limit;
    const products = await FoodCartModal.find(filter)
      .populate("_id", "item_name category_id item_image")
      .skip(skip)
      .limit(parseInt(limit))
      .sort({ createdAt: -1 });

    const total = await FoodCartModal.countDocuments(filter);
...
//food cart for resturant

export const foodaddCart = async (req, res) => {
  try {
    let {
      productId,
      productname,
    ...
    const parsedFinalAmount = Number(finalamount) || parsedPrice;
    ...
    const parsedStatus = status || "not yet submit";
    ...
    // ✅ Check if already in cart
    let existing = await FoodCartModal.findOne({
    ...
    if (existing) {
      ...
      existing.finalamount = parsedFinalAmount;
      existing.discount = parsedDiscount;
      existing.date_and_time = parsedDate;
    ...
    const newCartItem = new FoodCartModal({
      productId: productObjectId,
      productname,
      qty: qty || 1,
      userId: userObjectId,
...
export const foodupdateCart = async (req, res) => {
  try {
    let body = req.body;
    const updated = await Product.findByIdAndUpdate(req.params.id, body, { new: true });
    if (!updated)
...
export const foodincreaseQty = async (req, res) => {
  try {
    const { id } = req.params;

    let cartItem = await FoodCartModal.findById(id);
    if (!cartItem) {
      return res.status(404).json({ message: "Cart item not found" });
    }
...
export const fooddecreaseQty = async (req, res) => {
  try 
  {
    const { id } = req.params;
    let cartItem = await FoodCartModal.findById(id);
...
export const fooddeleteCart = async (req, res) => {
  try {
    const { id } = req.params;
    const product = await FoodCartModal.findByIdAndDelete(id);
    if (!product) return res.status(404).json({ success: false, message: "cart not found" });
    res.json({ success: true, message: "cart deleted" });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
