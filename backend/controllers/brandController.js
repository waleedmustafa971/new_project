...
export const addBrand = async (req, res) => {
    try {
        const { name,status,modulename } = req.body;
        ...
        const newBrand = new Brand({
            name,
            image: optimized[0] || "", modulename: modulename,
            status
        });
  ...
  try {
    ...
    const updated = await Brand.findByIdAndUpdate(
      id,
      {
        ...
        modulename: req.body.modulename || existing.modulename,
        image,
      },
      { new: true }
    );
...
export const listBrand = async (req, res) => {
  try {
    let { page = 1, limit = 10, name,modulename } = req.query;
