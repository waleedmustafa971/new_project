...
export const dashboardHome = async (req, res) => {
  try {
    ...
    // 2️⃣ Brands - Full collection
    const brands = await brandModal.find().sort({ createdAt: -1 });

    // 3️⃣ Categories - nested parent & subcategories
    const categoriesRaw = await Category.find().sort({ name: 1 }).lean();

    // Separate parents and children
    const parents = categoriesRaw.filter(cat => !cat.parentId);
    const subcategories = categoriesRaw.filter(cat => cat.parentId);

    // Attach subcategories to their parent
    const categories = parents.map(parent => ({
      ...parent,
      subcategories: subcategories.filter(sub => String(sub.parentId) === String(parent._id))
    }));

    // 4️⃣ Products grouped by showcasecategory (limit 10 each group)
...
export const dashboardHome__ = async (req, res) => {
  try {
    // 1️⃣ Sliders - Full collection
    const sliders = await Slider.find().sort({ createdAt: -1 });
    const brands = await brandModal.find().sort({ createdAt: -1 });
