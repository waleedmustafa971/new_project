...
export const getRecentstory_ = async (req, res) => {
 // console.log('log data ....' + req.query)
  try {
...
export const getPosts = async (req, res) => {
  try {
    ...
    const limit = req.query.limit;
    const skip = (page - 1) * limit;
    const loginUserId = req.query.username; //this is login userid userid
    const userid = req.query.userid; //this is login userid userid
    ...
    const processedReels = await Promise.all(
      reels.map(async (reel) => {
        ...
        //add this is for comments //loginUserId
