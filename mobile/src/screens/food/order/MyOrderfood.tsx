...
const MyOrderfood: React.FC = () => {
  ...
  const ORDER_TABS = ["Order Placed", "Processing", "Packed","Shiped","Out for Delivery","Reject","Delivered"];
  ...
  const fetchOrders = async () => {
    ...
    try {
      const res = await api.get(
        `/api/order/order-food-list?page=1&limit=100&userid=${user._id}&orderstatus=${status}`
      );
  ...
  const fetchTab = async (activeTab: string) => {
    ...
    try {
      const res = await api.get(
        `/api/order/order-food-list?page=1&limit=100&userid=${user._id}&orderstatus=${activeTab}`
      );
  ...
  /* ================= RENDER PRODUCT ================= */
  const renderProduct = (item: any, orderId: string, orderstatus: string) => (
    ...
    <View key={item._id} style={styles.productRow}>
      ...
      <View style={{ flex: 1 }}>
        <Text style={styles.productName}>
          {item.productId.item_name}
        </Text>
        <Text style={styles.productMeta}>
          Qty: {item.qty} | AED {item.final_price}
        </Text>
...
const styles = StyleSheet.create({
  ...
  tabContainer: {
    backgroundColor: "#fff", height: 5, padding: 10
  },
  tab: {
    paddingVertical: 8,
