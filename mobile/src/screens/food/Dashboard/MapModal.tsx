...
const MapModal = ({ visible, onClose, onLocationSelected, latitude, longitude, address: initialAddress }: any) => {
  ...
  const onMarkerDragEnd = async (e: any) => {
    console.log('change address : 111111');
    ...
    try {
      ...
      if (geoData.results.length > 0) {
        // setAddress(geoData.results[0].formatted_address);
        console.log('change address : ', geoData.results[0].formatted_address);
      }
    } catch (err) {
      console.log("Geocoding error:", err);
    } finally {
          ...
          <>
            <MapView
              ...
              onRegionChangeComplete={async (r) => {
                ...
                setMarker(newMarker);
                try {
                  const geoData = await Geocoder.from(r.latitude, r.longitude);
                  if (geoData.results.length > 0) {
                    setAddress(geoData.results[0].formatted_address);
                    console.log('test....', geoData.results[0].formatted_address);
                  }
                } catch (err) {
                  console.log("Geocoding error:", err);
                }
