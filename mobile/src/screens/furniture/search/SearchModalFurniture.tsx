...
const SearchModalClassified: React.FC<SearchModalClassifiedProps> = ({
  query: initialQuery = '',
  onClose,
}) => {
  ...
  useEffect(() => {
    ...
    db.transaction((tx: Transaction) => {
      tx.executeSql('CREATE TABLE IF NOT EXISTS recentclassified (id INTEGER PRIMARY KEY AUTOINCREMENT, term TEXT);');
    });
  ...
  const fetchRecent = () => {
    db.transaction((tx: Transaction) => {
      tx.executeSql(
        'SELECT * FROM recentclassified ORDER BY id DESC',
        [],
  ...
  const handleSearch = () => {
    ...
    db.transaction((tx: Transaction) => {
      tx.executeSql('INSERT INTO recentclassified (term) VALUES (?)', [query]);
    });
    ...
    try {
      const endpoint =
        selectedCategory === 'All'
          ? `${base.BASE_URL}/apis/property/recommandproperty/live?page=${page}&limit=${PAGE_LIMIT}&add_post=classified&search=${query}`
          : `${base.BASE_URL}/apis/property/recommandproperty/live?page=${page}&limit=${PAGE_LIMIT}&add_post=classified&Category=${selectedCategory}&search=${query}`;
      ...
      setFounddatalist((prev) => [...prev, ...json.users]);
    } catch (error) {
...
export default SearchModalClassified;
