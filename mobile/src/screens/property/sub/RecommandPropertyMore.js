  ...
  const fetchCategory = async () => {
    ...
    try {
      if (type == "Rooms For Rent") {
        const res = await fetch(base.BASE_URL + `/apis/property/recommandproperty/live?page=${page}&limit=${PAGE_LIMIT}&add_post=property&Category=Rooms For Rent`);
        const json = await res.json();
        setCategorydata((prev) => [...prev, ...json.users]);
        setTotalPages(json.totalPages);

      }
      else if (type == "Property for Rent") {
        const res = await fetch(base.BASE_URL + `/apis/property/recommandproperty/live?page=${page}&limit=${PAGE_LIMIT}&add_post=property&propertyType=Rent`);
        const json = await res.json();
        setCategorydata((prev) => [...prev, ...json.users]);
        setTotalPages(json.totalPages);
      }
      else if (type == "Property for Sale") {
        const res = await fetch(base.BASE_URL + `/apis/property/recommandproperty/live?page=${page}&limit=${PAGE_LIMIT}&add_post=property&propertyType=Sale`);
        const json = await res.json();
        setCategorydata((prev) => [...prev, ...json.users]);
        setTotalPages(json.totalPages);
      }
      else {
        const res = await fetch(base.BASE_URL + `/apis/property/recommandproperty/live?page=${page}&limit=${PAGE_LIMIT}&add_post=property`);
        const json = await res.json();
        setCategorydata((prev) => [...prev, ...json.users]);
        setTotalPages(json.totalPages);
      }
...
export default RecommandPropertyMore;
