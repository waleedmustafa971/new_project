import { View, Text, TouchableOpacity, Dimensions ,FlatList, Image, StyleSheet } from 'react-native';
...
const RoomforRent = ({ propertysalesdata }) => {
  ...
  const renderItem = ({ item, index }) => {
    ...
    return (
     ...
     {/*    <TouchableOpacity
        ...
        <TouchableOpacity
          style={styles.monthlybottom}
          onPress={() => toggleFavorite(item.id)}
        >
         <Text style={{ fontSize: 10, color: '#ffffff' }}>{item.rentispaid}</Text>
        </TouchableOpacity>
    ...
    <View style={{ marginTop: 10 }}>
       ...
       {/* Header */}
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 10 }}>
              <Text style={{ fontSize: 15, fontWeight: 'bold' }}>Room for Rent</Text>
      
              <TouchableOpacity
             //   onPress={() => navigation.navigate("KeeplookingPropertyMore")}
             onPress={() => {
              //RecommandPropertyMore
              navigation.navigate("RecommandPropertyMore",{
                type: "Rooms For Rent"
              })
             }}
                style={{
