...
const ChatDetails = () => {
    ...
    try {
      ...
      const response = await fetch(`${base.BASE_URL}/apis/mobile-not-get-message`, {
        ...
        body: JSON.stringify({
          me: me,
          partner: partner,
          type: type, //"private" or group params,
          convoId: partner
        })
      });

      const data = await response.json();
      console.log('syc from cloud', data)
      ...
      for (const msg of messages) {
        const formatted = {
          ...
          text: msg.text || "",
          imageUrl: JSON.stringify(msg.imageUrl),
        ...
        console.log('....cloud download.formate.....', result)
  ...
  const syncPendingMessages = async () => {
    try {
      ...
      for (const msg of pending) {
        ...
        const updatedatastatus = await updateMessageStatus(msg.id, "delivered"); // why this is not updateing
        console.log('update send status', updatedatastatus)
        await markDeliveredLocally(msg.id, me);
        setMessages(prev =>
          prev.map(m =>
            m._id === msg.id ? { ...m, status: "seen" } : m
          )
        );
      }
  ...
  const submit = async () => {
    ...
    console.log("Generated clientMessageId:", clientMessageId);
    const id = String(clientMessageId); // ✅ FIX: string ID
    ...
    const message = {
      ...
      audioUrl: "",
      videoUrl: "",
      status: "pending", // pending | sending | sent | delivered | seen | failed
      type,
      createdAt,
      msgByUserId: me,
      messagetype: 'text',
    ...
    console.log('...formate json ...', message)
    // 1️⃣ Add locally first

    // 1️⃣ Save locally FIRST
    //  await insertMessage(message);
    const datas = await insertMessage(message); // optional: only if not exists
    console.log("DB after insert:", datas);
    console.log("Inserting messagetype:", datas);
    setMessages(prev => {
  ...
  const sendWithAck = (msg: any) => {

    const payload = {
      clientMessageId: msg.id,   // ✅ permanent id
      convoId: msg.convoId,
      sender: msg.sender,
      receiver: msg.receiver,
    ...
    socket.current.emit("sendMessage", payload, async (ack) => {
      ...
      if (!ack?.success) {
        ...
        return;
          ...
          // Only update the existing message
      setMessages(prev =>
        prev.map(m =>
          m.id === msg.id
            ? { ...m, status: "sent", mongoId: ack.mongoId } // update in place
            : m
        )
      );

      // ✅ server saved message
      await updateMessageMongoId(msg.id, ack.mongoId); //updateMessageMongoId
      await updateMessageStatus(msg.id, "sent");
      console.log("Generated clientMessageId 2:", msg.id);
      ...
      /*  setMessages(prev => {
         ...
         if (exists) {
           // Update the existing message (status, mongoId, etc.)
           return prev.map(m =>
             m.id === msg.id
               ? { ...m, status: msg.status || m.status, mongoId: msg.mongoId || m.mongoId }
               : m
           );
         } else {
           return [...prev, msg]; // add if new
         }
       }); */
    setMessages(prev => {
    ...
    launchImageLibrary(
      ...
      async (response) => {
        if (response.assets && response.assets.length > 0) {
          ...
          if (!isOnline) return;
          syncPendingMessages();
          // Trigger background upload for pending multi-image messages
          /* const pending = await getPendingMessages();
            pending
             .filter(msg => msg.imageUrls && msg.status === "pending")
             .forEach(msg => uploadPendingImages(msg));  */
        }
      }
    );
  };
  ...
  const submitLocalImages = async (assets: any) => {
    ...
    const localMessage = {
      ...
      // ✅ Forward Support
      forwardedFrom: forwardMessage ? forwardMessage._id : null,
      isForwarded: forwardMessage ? true : false
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      // 1️⃣ Save to SQLite
      const savedata = await insertMessage(localAudioMessage);
      console.log('insert localAudioMessage......', savedata)
