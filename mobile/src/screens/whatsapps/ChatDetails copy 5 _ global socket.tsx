...
const ChatDetails = () => {
    ...
    try {
      ...
      for (const msg of messages) {
        ...
        socket?.emit("messageDelivered", {
          messageId: msg._id, // it will be mongodb auto _id
          userId: me
        });
      }
  ...
  useEffect(() => {
   ...
   /*  socket.on("newMessages", async (data: any) => {
    ...
    //   console.log('....incomming formatted.....', formatted)
      // Save locally
    ...
    socket.on("newMessages", async (data: any) => {
      if (!data?.messages?.length) return;
      // Loop through all incoming messages (if multiple)
      console.log('last receive',data.messages)
      for (const msg of data.messages) {
        // Use server _id as mongoId
        const clientMessageId = String(uuid.v4());
        console.log('receiv id', clientMessageId)
        const formatted = {
          id: String(clientMessageId),           // ✅ use server _id as unique id
          mongoId: String(msg._id),      // store mongoId separately
          sender: String(msg.msgByUserId),
          receiver: me,
          text: msg.text || "",
          ...
          audioUrl: msg.audioUrl || "",
          videoUrl: msg.videoUrl || "",
          status: "delivered",           // message received
          type: data.type || "private",
          createdAt: msg.createdAt,
          msgByUserId: String(msg.msgByUserId),
