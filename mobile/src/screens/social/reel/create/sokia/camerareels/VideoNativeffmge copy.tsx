...
export default function VideoNativeffmge({ videoUri, onBack }: any) {
  ...
  const handleExport_____ = async () => {
    try {
      ...
      console.log('start.... ', trimStart, '---end time----', trimEnd);
      console.log('duration....', duration);

      // ✅ Remove file:// prefix for C++ / FFmpeg
      ...
      // ✅ Play audio after confirming file exists sound is tested its working and playing
      /*  const sound = new Sound(localAudio, '', (error) => {
         if (error) {
         ...
         sound.play((success) => {
           ...
           sound.release(); // Always release after playback
         });
       }); */
      // ✅ Call C++ TurboModule
      /*  const result = await NativeSampleModule.processVideo(
         cleanFormattedUri,
         output,
         trimStart,
         duration,
      ...
      const result = NativeSampleModule.trimVideoMinimal(cleanFormattedUri, output, trimStart, duration);
