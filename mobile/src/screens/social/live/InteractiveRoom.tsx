...
const InteractiveRoom: React.FC<Props> = ({ route, navigation }) => {
  ...
  //  console.log('Inter......', JSON.stringify(hosterinfo))
    ...
    const [giftmessage, setGiftmessage] = useState(""); //messagesend
  ...
  //  const [remoteUid, setRemoteUid] = useState<number | null>(null);
          ...
          engine.current.registerEventHandler({
            ...
            onUserJoined: (connection, uid) => {
             ...
             /*  console.log("Remote Host joined with UID:", uid);
              setRemoteUid(uid); */
              // Now we add ALL users (Host and Co-hosts) to the array
    ...
    useEffect(() => {
        const initSocket = async () => {
            ...
            // 4. Critical Listener Placement
            socket.current.on("cohost-approved", (approvedUser) => {
                console.log('👍 APPROVE CO-HOSTER EVENT RECEIVED: ', JSON.stringify(approvedUser), ' | Frontend user ID: ', userid);
                
                // IMPORTANT: Check that approvedUser._id matches the correct user ID type
                if (approvedUser._id === userid) {
                    becomeCoHost(); // here we can implement Mic or Video Setup with Modal Popup
                }
            });
            ...
            {
            system: true,
            text: `${sender.name} sent ${gift.name} Coins ${gift.coinCost}`,
            ...
            // 💡 NEW LISTENER ADDED HERE: Handles the backend "gift-error"
            socket.current.on("gift-error", (error) => {
              console.error("❌ Gift Error Received:", error.message);
              setVisible(true);
            //  setMessagesend(error.message);
            ...
            // The error modal will appear, showing "Not enough coins"
            });
    ...
    }, [channelName, becomeCoHost, userid]);
 ...
 //   const coHostUids = remoteUid.slice(1);
  /*  
  this is only for host no co hoster
  const hostUid = remoteUid;
    const coHostUids = remoteUid; 
    */
// 💡 FIX 3: Separate Host UID (first in array) and Co-host UIDs (rest of the array)
    const hostUid = remoteUid.length > 0 ? remoteUid[0] : null;
    const coHostUids = remoteUid.slice(1);
    ...
    return (
        <View style={styles.container}>
            ...
            {
               giftmessage  === "Not enough coins" ?
               <>
                <RechargeModal show={showbalance} onHide={() => setShowBalance(false)}
                onSendGift={sendGift} />
               </> : null
            }
...
const styles = StyleSheet.create({
    ...
    debugTextOverlay: {
        ...
        color: 'white', backgroundColor: 'rgba(0,0,0,0.5)', padding: 5, fontSize: 14
    },
    loadingOverlay: {
    ...
    welcomeText: { color: 'white', fontSize: 12 },
